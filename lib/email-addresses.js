...
function parse5322(opts) {
    ...
    function _giveResult (ast, types) {
        function grabSemantic(n) {
            return n !== null ? n.semantic : null;
        }        
        var i, ret, addresses, addr, name, aspec, local, domain, group_addresses;
        if (ast === null) {
            return null;
        }
        ret = { ast: ast };
        addresses = [].concat.apply([], types.map(function (type) { return findAllNodes(type, ast) }));
        ret.addresses = [];
        for (i = 0; i < addresses.length; i += 1) {
            ...
            if (addr.name === 'group') {
                ...
                group_addresses = _giveResult(addr, ['mailbox']);
                ret.addresses.push({
                    node: addr,
                    parts: {
                        name: name,
                    ...
                    addresses: opts.simple ? group_addresses : group_addresses.addresses,
                });
                continue;
            }
            name = findNode('display-name', addr);
    ...
    function addrListGiveResult(ast) {
        return _giveResult(ast, ['address']);
    }
    
    function fromGiveResult (ast) {
        // Need to give: group + mailboxes under the group, plus any mailboxes or addresses not under a group
        // console.log(require('util').inspect(findAllNodes('mailbox', ast), {depth: 10, colors: true}))
        return _giveResult(ast, ['group', 'mailbox']);
    }
    
    function senderGiveResult (ast) {
        var ret = _giveResult(ast, ['group', 'mailbox']);
        return ret.addresses[0]; // Highlander - there is only one
    }
    ...
    var giveResMap = {
        'from': fromGiveResult,
        'sender': senderGiveResult,
        'reply-to': addrListGiveResult,
    };
    
    // console.log("Starting at: " + opts.startAt);
    
    var startAt = startAtMap[opts.startAt] || addressList;
    var giveResult = giveResMap[opts.startAt] || addrListGiveResult;

    if (!opts.strict) {
        initialize();
        opts.strict = true;
        parsed = startAt(parseString);
...
function parseOneAddressSimple(opts) {
    ...
    var result = parse5322(opts);
    // console.log(require('util').inspect(result, {depth: 4, colors: true}));
